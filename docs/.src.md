...
1. [Reference](#9-reference)
   ...
   1. [Environment variables](#94-environment-variables)
      ...
      1. [`ARGPARSER_DEBUG`](#9418-argparser_debug)
      1. [`ARGPARSER_ERROR_EXIT_CODE`](#9419-argparser_error_exit_code)
      1. [`ARGPARSER_ERROR_STYLE`](#9420-argparser_error_style)
      1. [`ARGPARSER_HELP_ARG_GROUP`](#9421-argparser_help_arg_group)
      1. [`ARGPARSER_HELP_DESCRIPTION`](#9422-argparser_help_description)
      1. [`ARGPARSER_HELP_EXIT_CODE`](#9423-argparser_help_exit_code)
      1. [`ARGPARSER_HELP_FILE`](#9424-argparser_help_file)
      1. [`ARGPARSER_HELP_FILE_INCLUDE_CHAR`](#9425-argparser_help_file_include_char)
      1. [`ARGPARSER_HELP_FILE_KEEP_COMMENTS`](#9426-argparser_help_file_keep_comments)
      1. [`ARGPARSER_HELP_OPTIONS`](#9427-argparser_help_options)
      1. [`ARGPARSER_HELP_STYLE`](#9428-argparser_help_style)
      1. [`ARGPARSER_LANGUAGE`](#9429-argparser_language)
      1. [`ARGPARSER_MAX_COL_WIDTH_1`](#9430-argparser_max_col_width_1)
      1. [`ARGPARSER_MAX_COL_WIDTH_2`](#9431-argparser_max_col_width_2)
      1. [`ARGPARSER_MAX_COL_WIDTH_3`](#9432-argparser_max_col_width_3)
      1. [`ARGPARSER_MAX_WIDTH`](#9433-argparser_max_width)
      1. [`ARGPARSER_POSITIONAL_ARG_GROUP`](#9434-argparser_positional_arg_group)
      1. [`ARGPARSER_READ_ARGS`](#9435-argparser_read_args)
      1. [`ARGPARSER_SCRIPT_NAME`](#9436-argparser_script_name)
      1. [`ARGPARSER_SET_ARGS`](#9437-argparser_set_args)
      1. [`ARGPARSER_SET_ARRAYS`](#9438-argparser_set_arrays)
      1. [`ARGPARSER_SILENCE_ERRORS`](#9439-argparser_silence_errors)
      1. [`ARGPARSER_SILENCE_WARNINGS`](#9440-argparser_silence_warnings)
      1. [`ARGPARSER_TRANSLATION_FILE`](#9441-argparser_translation_file)
      1. [`ARGPARSER_UNSET_ARGS`](#9442-argparser_unset_args)
      1. [`ARGPARSER_UNSET_ENV_VARS`](#9443-argparser_unset_env_vars)
      1. [`ARGPARSER_UNSET_FUNCTIONS`](#9444-argparser_unset_functions)
      1. [`ARGPARSER_USAGE_EXIT_CODE`](#9445-argparser_usage_exit_code)
      1. [`ARGPARSER_USAGE_FILE`](#9446-argparser_usage_file)
      1. [`ARGPARSER_USAGE_FILE_INCLUDE_CHAR`](#9447-argparser_usage_file_include_char)
      1. [`ARGPARSER_USAGE_FILE_KEEP_COMMENTS`](#9448-argparser_usage_file_keep_comments)
      1. [`ARGPARSER_USAGE_MESSAGE_OPTION_TYPE`](#9449-argparser_usage_message_option_type)
      1. [`ARGPARSER_USAGE_MESSAGE_ORIENTATION`](#9450-argparser_usage_message_orientation)
      1. [`ARGPARSER_USAGE_OPTIONS`](#9451-argparser_usage_options)
      1. [`ARGPARSER_USAGE_STYLE`](#9452-argparser_usage_style)
      1. [`ARGPARSER_USE_LONG_OPTIONS`](#9453-argparser_use_long_options)
      1. [`ARGPARSER_USE_SHORT_OPTIONS`](#9454-argparser_use_short_options)
      1. [`ARGPARSER_USE_STYLES_IN_FILES`](#9455-argparser_use_styles_in_files)
      1. [`ARGPARSER_VERSION_EXIT_CODE`](#9456-argparser_version_exit_code)
      1. [`ARGPARSER_VERSION_NUMBER`](#9457-argparser_version_number)
      1. [`ARGPARSER_VERSION_OPTIONS`](#9458-argparser_version_options)
      1. [`ARGPARSER_VERSION_STYLE`](#9459-argparser_version_style)
      1. [`ARGPARSER_WARNING_STYLE`](#9460-argparser_warning_style)
      1. [`ARGPARSER_WRITE_ARGS`](#9461-argparser_write_args)
...
- `arg_group`: the argument group for grouping of keyword arguments in the help text (like `"Optional options"` for `var_4`, default: [`ARGPARSER_POSITIONAL_ARG_GROUP`](#9434-argparser_positional_arg_group))
...
The help message details all short and long option names, their optionality (*i.e.*, whether there are default values), and their choice values, using the same syntax as in the usage message (square brackets for optional arguments, curly braces for choice values). Additionally, the help text and notes from the arguments definition are given. The arguments are separated by their groups, thus structuring the help message. First, the group for the positional arguments is given (indicated by [`ARGPARSER_POSITIONAL_ARG_GROUP`](#9434-argparser_positional_arg_group)), then follow the keyword argument groups in alphabetical order. Finally, the default `--help`, `--usage`, and `--version` arguments (the latter for the [version message](#58-version-messages)) are given as separate, yet unnamed group.

The help message's structure aims at reproducing the commonly found structure in command-line programs. By setting [`ARGPARSER_MAX_COL_WIDTH_1`](#9430-argparser_max_col_width_1), [`ARGPARSER_MAX_COL_WIDTH_2`](#9431-argparser_max_col_width_2), or [`ARGPARSER_MAX_COL_WIDTH_3`](#9432-argparser_max_col_width_3), the column widths may be adapted to your needs, recommendably totalling 77 characters (thus 79 characters including the separating spaces). Note that columns are automatically shrunk, when their content is narrower, but they're not expanded, when their content is wider. This is to guarantee that the help message, when *e.g.* sent as logging output, nicely fits in the space you have.

Alternatively, you may want to set [`ARGPARSER_MAX_WIDTH`](#9433-argparser_max_width). By this, the help message will have a defined width, independent of shrunk columns. This is achieved by expanding the third column (with the help text) to the remaining width. For this to work, [`ARGPARSER_MAX_COL_WIDTH_3`](#9432-argparser_max_col_width_3) must be set to `0`.
...
For a better overview when having lots of arguments, we can choose a columnar layout instead of the single row, using [`ARGPARSER_USAGE_MESSAGE_ORIENTATION`](#9450-argparser_usage_message_orientation):
...
Additionally, we may choose to show the long options by [`ARGPARSER_USAGE_MESSAGE_OPTION_TYPE`](#9449-argparser_usage_message_option_type):
...
Additionally, we need a separate file, which we'll call [`help_message.txt`](../resources/help_message.txt) and have passed as value to [`ARGPARSER_HELP_FILE`](#9424-argparser_help_file). This plain-text file stores the help message's structure and can contain arbitrary additional content.
...
Thereby, `<ArgumentGroup>` can be the name of any argument group given in the arguments definition, like `"Mandatory options"` for the include directive `@Mandatory options` or `"Optional options"` for the include directive `@Optional options`. `@Description` prints the [`ARGPARSER_HELP_DESCRIPTION`](#9422-argparser_help_description), `@Help` prints the help, usage, and version options (depending on which of them are defined by [`ARGPARSER_ADD_HELP`](#942-argparser_add_help), [`ARGPARSER_ADD_USAGE`](#943-argparser_add_usage), or [`ARGPARSER_ADD_VERSION`](#944-argparser_add_version)), `@Remark` prints the remark that mandatory arguments to long options are mandatory for short options too, and `@Usage` prints the usage line. Finally, the shorthand `@All` means to print the usage line, the description, the remark, all argument groups, and the help options, in this order, while `@Header` prints the usage line, the description, and the remark.

Further, lines starting with a `"#"` character in the help file aren't printed if [`ARGPARSER_HELP_FILE_KEEP_COMMENTS`](#9426-argparser_help_file_keep_comments) is set to `false` (the default). This allows you to comment your help file, perhaps to explain the structure&mdash;or just to write a header or footer with your name and debug email address inside.

The same as for help messages can be done for usage messages, using the [`ARGPARSER_USAGE_FILE`](#9446-argparser_usage_file), [`ARGPARSER_USAGE_FILE_INCLUDE_CHAR`](#9447-argparser_usage_file_include_char), and [`ARGPARSER_USAGE_FILE_KEEP_COMMENTS`](#9448-argparser_usage_file_keep_comments) environment variables. However there, only the `@All` directive is supported, yet.
<!-- </section> -->

<!-- <section file="tutorial/help_and_usage_message_localization.md"> -->
### 5.7. Help and usage message localization

It is even possible to localize your script's help and usage message. For the usage message, all you need is an [`ARGPARSER_TRANSLATION_FILE`](#9441-argparser_translation_file), a simplified [YAML](https://en.wikipedia.org/wiki/YAML "wikipedia.org &rightarrow; YAML") file giving the translation of the auto-generated parts in the messages. For each section, you give the language identifier for the language you want the message to be translated to, *i.e.*, the [`ARGPARSER_LANGUAGE`](#9429-argparser_language). For the usage message, this suffices, but in the help message, also non-auto-generated parts are included, especially each argument's help text. For them to be translated, you need a dedicated [`ARGPARSER_ARG_DEF_FILE`](#9410-argparser_arg_def_file) and possibly a localized [`ARGPARSER_HELP_FILE`](#9424-argparser_help_file).
...
Besides the [`ARGPARSER_HELP_OPTIONS`](#9427-argparser_help_options), `--help`, the [`ARGPARSER_USAGE_OPTIONS`](#9451-argparser_usage_options), and `--usage`, there is a third option intended to help the user, the [`ARGPARSER_VERSION_OPTIONS`](#9458-argparser_version_options) (default: `-V`) and `--version`. This flag compiles a brief version message for your script, showing its canonical name (the [`ARGPARSER_SCRIPT_NAME`](#9436-argparser_script_name)) and the version number (the [`ARGPARSER_VERSION_NUMBER`](#9457-argparser_version_number)). Just as for the help and usage messages, you can disable the version message (and its corresponding flags) by setting [`ARGPARSER_ADD_VERSION`](#944-argparser_add_version) to `false`. Note that the default short option name is an uppercase `"V"`, such that you can use the lowercase `"v"` (as `-v`) for your purposes, like `--verbatim` or `--verbose`. This is in line with the common behavior of command-line programs. By setting `ARGPARSER_VERSION_OPTIONS` accordingly, you can of course change it to your needs, if desired.
...
The Argparser outputs about a hundred different error and warning messages to give both you and your script's user as detailled feedback as possible about what went wrong with the argument parsing, value checking, *etc.* Each message starts with your script's canonical name (the [`ARGPARSER_SCRIPT_NAME`](#9436-argparser_script_name)), followed by either `"Error:"` or `"Warning:"` and the respective message. Using the same simplified [YAML](https://en.wikipedia.org/wiki/YAML "wikipedia.org &rightarrow; YAML") file as for the help and usage messages (the [`ARGPARSER_TRANSLATION_FILE`](#9441-argparser_translation_file)), also the error and warning messages can be fully localized.
...
The argument identifier must be a valid variable identifier in Bash syntax (at least when [`ARGPARSER_SET_ARGS`](#9437-argparser_set_args) is set to `true`, else, the ID is only used as key in associative arrays). These are defined as a word beginning with an alphabetic character or an underscore, followed by an arbitrary number of alphanumeric characters or underscores. In Bash's extglob syntax, the regular expression for verification may look like `[[:alpha:]_]*([[:word:]])`, assuming C locale.
...
The argument groups serve to group some arguments in the help message. One group shall comprise all positional arguments (if any is defined) and is named by [`ARGPARSER_POSITIONAL_ARG_GROUP`](#9434-argparser_positional_arg_group). Any other argument group shall only contain keyword arguments (options), and is sorted alphabetically in the help message. In the future, argument groups might be expanded to allow actual grouping of arguments upon parsing, such that options may only be given together or mutually exclusively.
...
The help text should consist of a terse summary of the argument's function, like turning a feature on or off (which may be accomplished by flags), what a file is used for (like for input or output), or how your script's output may be modified. A help text can take any form, but be wary of it being wrapped to fit the width of the third column in the help message (if [`ARGPARSER_MAX_COL_WIDTH_3`](#9432-argparser_max_col_width_3) is non-zero) or the total line length ([`ARGPARSER_MAX_WIDTH`](#9433-argparser_max_width)). Help messages are no replacement for the manual, so the help text shouldn't be overly verbose.
<!-- </section> -->

<!-- <section file="reference/colors_and_styles.md"> -->
### 9.2. Colors and styles

The Argparser employs [Select Graphic Rendition (SGR) ANSI escape sequence codes](https://en.wikipedia.org/wiki/ANSI_escape_code#Select_Graphic_Rendition_parameters "wikipedia.org &rightarrow; ANSI escape code &rightarrow; Select Graphic Rendition parameters") to set the appearance of help, usage, version, error, and warning messages. To this end, five environment variable are defined, *viz.* [`ARGPARSER_HELP_STYLE`](#9428-argparser_help_style), [`ARGPARSER_USAGE_STYLE`](#9452-argparser_usage_style), [`ARGPARSER_VERSION_STYLE`](#9459-argparser_version_style), [`ARGPARSER_ERROR_STYLE`](#9420-argparser_error_style), and [`ARGPARSER_WARNING_STYLE`](#9460-argparser_warning_style). Since the escape codes are nonprintable, not all terminals or text editors may support them. Many terminals do, while *e.g.* [`less`](https://man7.org/linux/man-pages/man1/less.1.html "man7.org &rightarrow; man pages &rightarrow; less(1)") has a dedicated flag, `--raw-control-chars`.

When [`ARGPARSER_USE_STYLES_IN_FILES`](#9455-argparser_use_styles_in_files) is set to `false`, the escape sequences are only included when `STDOUT`/`STDERR` is a terminal, keeping files plain 7-bit ASCII for simpler parsing. Note that, when your arguments definition or help file includes non-ASCII characters (as is usual for almost any language other than English varieties), the output contains these characters as well.
...
Six section names (include directives) are supported in the help files, and one (`@All`) in the usage files. These are introduced with the [`ARGPARSER_HELP_FILE_INCLUDE_CHAR`](#9425-argparser_help_file_include_char) or [`ARGPARSER_USAGE_FILE_INCLUDE_CHAR`](#9447-argparser_usage_file_include_char), respectively, defaulting to `"@"`.
...
Consequently, the help message generated from the [`ARGPARSER_HELP_FILE`](#9424-argparser_help_file) with the following content:
...
The `@<ArgumentGroup>` directive prints the help message block for the respective `"<ArgumentGroup>"`, like `"Mandatory options"` for the include directive `@Mandatory options` or `"Optional options"` for the include directive `@Optional options`. Their order in the auto-generated help message would be alphabetical for the keyword arguments, preceded by the group for the positional arguments (the [`ARGPARSER_POSITIONAL_ARG_GROUP`](#9434-argparser_positional_arg_group)). Thus, if you have reasons for another structure, you need an [`ARGPARSER_HELP_FILE`](#9424-argparser_help_file), denoting all arguments groups in the order preferred by you.

#### 9.3.3. `@Description` directive

The `@Description` directive prints the contents of [`ARGPARSER_HELP_DESCRIPTION`](#9422-argparser_help_description), wrapped to fit the help message's [`ARGPARSER_MAX_WIDTH`](#9433-argparser_max_width). If `ARGPARSER_HELP_DESCRIPTION` is empty (the default), nothing is printed.
...
The `@Usage` directive prints the line `Usage: <script_name> ...`, with `<script_name>` replaced by [`ARGPARSER_SCRIPT_NAME`](#9436-argparser_script_name), defaulting to your script's name. This should be given as first line.
<!-- </section> -->
...
| [`ARGPARSER_DEBUG`](#9418-argparser_debug)                                         | *bool*     | `false`                  |
| [`ARGPARSER_ERROR_EXIT_CODE`](#9419-argparser_error_exit_code)                     | *int*      | `1`                      |
| [`ARGPARSER_ERROR_STYLE`](#9420-argparser_error_style)                             | *str*      | `"red,bold,reverse"`     |
| [`ARGPARSER_HELP_EXIT_CODE`](#9423-argparser_help_exit_code)                       | *int*      | `0`                      |
| [`ARGPARSER_HELP_ARG_GROUP`](#9421-argparser_help_arg_group)                       | *str*      | `"Help options"`         |
| [`ARGPARSER_HELP_FILE`](#9424-argparser_help_file)                                 | *file*     | `""`                     |
| [`ARGPARSER_HELP_FILE_INCLUDE_CHAR`](#9425-argparser_help_file_include_char)       | *char*     | `"@"`                    |
| [`ARGPARSER_HELP_FILE_KEEP_COMMENTS`](#9426-argparser_help_file_keep_comments)     | *bool*     | `false`                  |
| [`ARGPARSER_HELP_OPTIONS`](#9427-argparser_help_options)                           | *char*     | `"h,?"`                  |
| [`ARGPARSER_HELP_STYLE`](#9428-argparser_help_style)                               | *str*      | `"italic"`               |
| [`ARGPARSER_LANGUAGE`](#9429-argparser_language)                                   | *str*      | `"en"`                   |
| [`ARGPARSER_MAX_COL_WIDTH_1`](#9430-argparser_max_col_width_1)                     | *uint*     | `9`[^19]                 |
| [`ARGPARSER_MAX_COL_WIDTH_2`](#9431-argparser_max_col_width_2)                     | *uint*     | `33`[^19]                |
| [`ARGPARSER_MAX_COL_WIDTH_3`](#9432-argparser_max_col_width_3)                     | *uint*     | `0`[^19]                 |
| [`ARGPARSER_MAX_WIDTH`](#9433-argparser_max_width)                                 | *uint*     | `79`                     |
| [`ARGPARSER_POSITIONAL_ARG_GROUP`](#9434-argparser_positional_arg_group)           | *str*      | `"Positional arguments"` |
| [`ARGPARSER_READ_ARGS`](#9435-argparser_read_args)                                 | *bool*     | `true`                   |
| [`ARGPARSER_SCRIPT_NAME`](#9436-argparser_script_name)                             | *str*      | `"${0##*/}"`             |
| [`ARGPARSER_SET_ARGS`](#9437-argparser_set_args)                                   | *bool*     | `true`                   |
| [`ARGPARSER_SET_ARRAYS`](#9438-argparser_set_arrays)                               | *bool*     | `true`                   |
| [`ARGPARSER_SILENCE_ERRORS`](#9439-argparser_silence_errors)                       | *bool*     | `false`                  |
| [`ARGPARSER_SILENCE_WARNINGS`](#9440-argparser_silence_warnings)                   | *bool*     | `false`                  |
| [`ARGPARSER_TRANSLATION_FILE`](#9441-argparser_translation_file)                   | *file*     | `""`                     |
| [`ARGPARSER_UNSET_ARGS`](#9442-argparser_unset_args)                               | *bool*     | `true`                   |
| [`ARGPARSER_UNSET_ENV_VARS`](#9443-argparser_unset_env_vars)                       | *bool*     | `true`                   |
| [`ARGPARSER_UNSET_FUNCTIONS`](#9444-argparser_unset_functions)                     | *bool*     | `true`                   |
| [`ARGPARSER_USAGE_EXIT_CODE`](#9445-argparser_usage_exit_code)                     | *int*      | `0`                      |
| [`ARGPARSER_USAGE_FILE`](#9446-argparser_usage_file)                               | *file*     | `""`                     |
| [`ARGPARSER_USAGE_FILE_INCLUDE_CHAR`](#9447-argparser_usage_file_include_char)     | *char*     | `"@"`                    |
| [`ARGPARSER_USAGE_FILE_KEEP_COMMENTS`](#9448-argparser_usage_file_keep_comments)   | *bool*     | `false`                  |
| [`ARGPARSER_USAGE_MESSAGE_OPTION_TYPE`](#9449-argparser_usage_message_option_type) | *str*      | `"short"`                |
| [`ARGPARSER_USAGE_MESSAGE_ORIENTATION`](#9450-argparser_usage_message_orientation) | *str*      | `"row"`                  |
| [`ARGPARSER_USAGE_OPTIONS`](#9451-argparser_usage_options)                         | *char*     | `"u"`                    |
| [`ARGPARSER_USAGE_STYLE`](#9452-argparser_usage_style)                             | *str*      | `"italic"`               |
| [`ARGPARSER_USE_LONG_OPTIONS`](#9453-argparser_use_long_options)                   | *bool*     | `true`                   |
| [`ARGPARSER_USE_SHORT_OPTIONS`](#9454-argparser_use_short_options)                 | *bool*     | `true`                   |
| [`ARGPARSER_USE_STYLES_IN_FILES`](#9455-argparser_use_styles_in_files)             | *bool*     | `false`                  |
| [`ARGPARSER_VERSION_EXIT_CODE`](#9456-argparser_version_exit_code)                 | *int*      | `0`                      |
| [`ARGPARSER_VERSION_NUMBER`](#9457-argparser_version_number)                       | *str*      | `"1.0.0"`                |
| [`ARGPARSER_VERSION_OPTIONS`](#9458-argparser_version_options)                     | *char*     | `"V"`                    |
| [`ARGPARSER_VERSION_STYLE`](#9459-argparser_version_style)                         | *str*      | `"bold"`                 |
| [`ARGPARSER_WARNING_STYLE`](#9460-argparser_warning_style)                         | *str*      | `"red,bold"`             |
| [`ARGPARSER_WRITE_ARGS`](#9461-argparser_write_args)                               | *bool*     | `false`                  |
...
[^19]: Sum of values is recommended to be 77, except when [`ARGPARSER_MAX_COL_WIDTH_3`](#9432-argparser_max_col_width_3) is `0`.
<!-- </section> -->
...
- ***Description:*** The name of the positional arguments' argument group in the arguments definition, to be shown in the help message, *i.e.*, the translated [`ARGPARSER_POSITIONAL_ARG_GROUP`](#9434-argparser_positional_arg_group).

#### 9.5.2. `Help options`

- ***Message:*** `Help options`
- ***Description:*** The name of the help options' argument group in the arguments definition, to be shown in the help message, *i.e.*, the translated [`ARGPARSER_HELP_ARG_GROUP`](#9421-argparser_help_arg_group).

#### 9.5.3. `Help description`

- ***Message:*** `""`
- ***Description:*** The script's description, indicating its purpose in the help message, *i.e.*, the translated [`ARGPARSER_HELP_DESCRIPTION`](#9422-argparser_help_description). The value is empty by default, and hence also the translations.
...
- ***Description:*** the help text for the help options, *i.e.*, the [`ARGPARSER_HELP_OPTIONS`](#9427-argparser_help_options) and `--help`, if [`ARGPARSER_ADD_HELP`](#942-argparser_add_help) is set to `true`.

#### 9.5.13. `--usage`

- ***Message:*** `display the usage and exit`
- ***Description:*** the help text for the usage options, *i.e.*, the [`ARGPARSER_USAGE_OPTIONS`](#9451-argparser_usage_options) and `--usage`, if [`ARGPARSER_ADD_USAGE`](#943-argparser_add_usage) is set to `true`.

#### 9.5.14. `--version`

- ***Message:*** `display the version and exit`
- ***Description:*** the help text for the version options, *i.e.*, the [`ARGPARSER_VERSION_OPTIONS`](#9458-argparser_version_options) and `--version`, if [`ARGPARSER_ADD_VERSION`](#944-argparser_add_version) is set to `true`.
...
- ***Description:*** The error that the environment variable [`ARGPARSER_USAGE_MESSAGE_OPTION_TYPE`](#9449-argparser_usage_message_option_type) is not set to `long` or `short`.
...
- ***Description:*** The error that the environment variable [`ARGPARSER_USAGE_MESSAGE_ORIENTATION`](#9450-argparser_usage_message_orientation) is not set to `row` or `column`.
...
- ***Description:*** The error that the environment variable [`ARGPARSER_USAGE_MESSAGE_OPTION_TYPE`](#9532-error-env-var-option-type) requests short option names, while [`ARGPARSER_USE_SHORT_OPTIONS`](#9454-argparser_use_short_options) prohibits this.
...
- ***Description:*** The error that the environment variable [`ARGPARSER_USAGE_MESSAGE_OPTION_TYPE`](#9532-error-env-var-option-type) requests long option names, while [`ARGPARSER_USE_LONG_OPTIONS`](#9453-argparser_use_long_options) prohibits this.
...
- ***Interpolated variables:***
  - `$1`: The path to the Argparser, as [`ARGPARSER_SCRIPT_NAME`](#9436-argparser_script_name).
...
- ***Interpolated variables:***
  - `$1`: The [`ARGPARSER_SCRIPT_NAME`](#9436-argparser_script_name) with the problematic definition.
...
- ***Reasons for error:*** When parsing the [`ARGPARSER_TRANSLATION_FILE`](#9441-argparser_translation_file), the Argparser found a YAML line that it doesn't recognize. This is most likely due to a YAML feature that the Argparser doesn't support to keep the parser simple.
- ***Interpolated variables:***
  - `$1`: The YAML line.
...
- ***Reasons for error:*** When parsing the [`ARGPARSER_HELP_FILE`](#9424-argparser_help_file) or [`ARGPARSER_USAGE_FILE`](#9446-argparser_usage_file), the Argparser found an include directive that is not supported.
- ***Interpolated variables:***
...
- ***Reasons for warning:*** When trying to translate a string from the [`ARGPARSER_TRANSLATION_FILE`](#9441-argparser_translation_file), the Argparser found that the identifier is missing in the dictionary, and by this, also in the translation file. Thus, it uses the untranslated English string as fallback.
- ***Interpolated variables:***
  - `$1`: The [`ARGPARSER_TRANSLATION_FILE`](#9441-argparser_translation_file).
...
- ***Reasons for warning:*** When trying to translate a string from the [`ARGPARSER_TRANSLATION_FILE`](#9441-argparser_translation_file), the Argparser found that the translation to the [`ARGPARSER_LANGUAGE`](#9429-argparser_language) for the identifier is missing in the dictionary, and by this, also in the translation file. Thus, it uses the untranslated English string as fallback.
- ***Interpolated variables:***
  - `$1`: The [`ARGPARSER_TRANSLATION_FILE`](#9441-argparser_translation_file).
  - `$2`: The [`ARGPARSER_LANGUAGE`](#9429-argparser_language).
