...
1. [Roadmap](#6-roadmap)
   1. [Future enhancements](#61-future-enhancements)
      ...
      1. [POSIX compliance](#616-posix-compliance)
      1. [Alternative option prefixes (`+`/`/`)](#617-alternative-option-prefixes-)
...
Additionally, we need a separate file, which we'll call `help_message.txt` and have passed as value to [`ARGPARSER_HELP_FILE`](#8425-argparser_help_file). This plain-text file stores the help message's structure and can contain arbitrary additional content.
...
- [`@Description`](#834-description-directive)
- [`@Header`](#833-header-directive)
- [`@Help`](#837-help-directive)
- [`@Remark`](#835-remark-directive)
- [`@Usage`](#836-usage-directive)
...
The argument groups serve to group arguments in the help message. The first group shall comprise all positional arguments (if any is defined) and is named by [`ARGPARSER_POSITIONAL_ARG_GROUP`](#8435-argparser_positional_arg_group). Any other argument group shall only contain keyword arguments (options), and is sorted alphabetically in the help message. In the future, argument groups will be expanded to allow actual grouping of arguments upon parsing, such that options may only be given together or mutually exclusively.
...
The Argparser employs [Select Graphic Rendition (SGR) ANSI escape sequence codes](https://en.wikipedia.org/wiki/ANSI_escape_code#Select_Graphic_Rendition_parameters "wikipedia.org &rightarrow; ANSI escape code &rightarrow; Select Graphic Rendition parameters") to set the appearance of error, warning, help, usage, and version messages. To this end, five environment variable are defined, *viz.*, [`ARGPARSER_HELP_STYLE`](#8429-argparser_help_style), [`ARGPARSER_USAGE_STYLE`](#8454-argparser_usage_style), [`ARGPARSER_VERSION_STYLE`](#8461-argparser_version_style), [`ARGPARSER_ERROR_STYLE`](#8421-argparser_error_style), and [`ARGPARSER_WARNING_STYLE`](#8462-argparser_warning_style). Since the escape codes are nonprintable, not any terminal or text editor may support them. Many terminals do, while *e.g.* [`less`](https://man7.org/linux/man-pages/man1/less.1.html "man7.org &rightarrow; man pages &rightarrow; less(1)") has a dedicated flag, `--raw-control-chars`.
...
Six section names (include directives) are supported in the help and usage files. These are introduced with the [`ARGPARSER_HELP_FILE_INCLUDE_CHAR`](#8426-argparser_help_file_include_char) or [`ARGPARSER_USAGE_FILE_INCLUDE_CHAR`](#8449-argparser_usage_file_include_char), respectively, defaulting to `"@"`.
...
- ***Description:*** The remark that mandatory arguments to long options are mandatory for short options too, to be used in the help message for the [`@Remark`](#835-remark-directive) include directive.
...
- ***Interpolated variables:***
  - `$1`: The [`ARGPARSER_TRANSLATION_FILE`](#8443-argparser_translation_file).
  - `$2`: The [`ARGPARSER_LANGUAGE_`](#8430-argparser_language).
